...
2. If it exists, read it and add entries for the changes being committed under the `[Unreleased]` section, following the CHANGELOG Maintenance section below.
3. If it does not exist, create it with the standard header and an `[Unreleased]` section containing entries for the current changes.
4. Classify entries as Added, Changed, Fixed, Removed, Deprecated, or Security.
5. Skip formatting-only changes, trivial refactors, or dependency bumps unless user-visible.
6. Stage the updated CHANGELOG.md with `git add CHANGELOG.md`.
- **Validation:** CHANGELOG.md is staged and contains entries for the current commit.

### Step 4: Run Formatter and Re-stage
...
- [ ] CHANGELOG.md updated with entries for this commit (under [Unreleased]).
